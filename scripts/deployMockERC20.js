...
module.exports = async function(callback) {
  try {
    const options = yargs
      .usage('Usage: --name <name> --symbol <symbol> --decimals <decimals> --permit <permit> --gas <gasPrice>')
    ...
    const tx = options.permit
      ? await PermitERC20.new(options.name, options.symbol, options.decimals, {gasPrice: options.gas})
      : await ERC20.new(options.name, options.symbol, options.decimals, {gasPrice: options.gas})
