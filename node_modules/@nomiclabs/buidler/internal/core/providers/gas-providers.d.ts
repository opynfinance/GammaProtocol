import { IEthereumProvider } from "../../../types";
export declare const GANACHE_GAS_MULTIPLIER = 5;
export declare function createFixedGasProvider(provider: IEthereumProvider, gasLimit: number): import("../../../types").EthereumProvider;
export declare function createFixedGasPriceProvider(provider: IEthereumProvider, gasPrice: number): import("../../../types").EthereumProvider;
export declare function createAutomaticGasProvider(provider: IEthereumProvider, gasMultiplier?: number): import("../../../types").EthereumProvider;
export declare function createAutomaticGasPriceProvider(provider: IEthereumProvider): import("../../../types").EthereumProvider;
/**
 * This provider multiplies whatever gas estimation Ganache gives by [[GANACHE_GAS_MULTIPLIER]]
 *
 * NOTE: This bug was present at least in Ganache 6.4.x.
 * One way to test if the bug is still present is to check if the estimation to
 * run a deployment transaction with this data is high enough:
 *  * 
 */
export declare function createGanacheGasMultiplierProvider(provider: IEthereumProvider): import("../../../types").EthereumProvider;
//# sourceMappingURL=gas-providers.d.ts.map