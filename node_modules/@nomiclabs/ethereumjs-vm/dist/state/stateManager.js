...
class StateManager {
     ...
     * @param key - Key in the account's storage to get the value for. Must be 32 bytes long.
     */
    getOriginalContractStorage(address, key, cb) {
        if (key.length !== 32) {
            return cb(new Error('Storage key must be 32 bytes long'));
        }
        ...
        if (map.has(keyHex)) {
            ...
            this.getContractStorage(address, key, (err, current) => {
                if (err)
                    return cb(err);
    ...
    generateCanonicalGenesis(cb) {
        ...
        this.hasGenesisState((err, genesis) => {
            if (!genesis && !err) {
                this.generateGenesis(genesisStates_1.genesisStateByName(this._common.chainName()), cb);
            }
            else {
                cb(err);
            }
    ...
    generateGenesis(initState, cb) {
        ...
        asyncLib.eachSeries(addresses, (address, done) => {
            const account = new ethereumjs_account_1.default();
    ...
    cleanupTouchedAccounts(cb) {
        ...
        asyncLib.forEach(touchedArray, (addressHex, next) => {
