...
function allocateCalldataAndReturndata(abiEntry, contractNode, referenceDeclarations, userDefinedTypes, abiAllocations, compilationId, compiler, constructorContext) {
    ...
    switch (abiEntry.type) {
        ...
        case "function":
            ...
            if (contractNode) {
                ...
                node = linearizedBaseContracts.reduce((foundNode, baseContractId) => {
                    if (foundNode !== undefined) {
                        return foundNode; //once we've found something, we don't need to keep looking
                    }
                    let baseContractNode = baseContractId === contractNode.id
                    ...
                    functionNode => AbiDataUtils.definitionMatchesAbi(abiEntry, functionNode, referenceDeclarations));
...
function allocateEvent(abiEntry, contractNode, referenceDeclarations, userDefinedTypes, abiAllocations, compilationId, compiler) {
    ...
    if (contractNode) {
        ...
        if (!node) {
            ...
            node = linearizedBaseContractsMinusSelf.reduce((foundNode, baseContractId) => {
                ...
                return baseContractNode.nodes.find(
                //may be undefined! that's OK!
                eventNode => AbiDataUtils.definitionMatchesAbi(
...
function getCalldataAllocationsForContract(abi, contractNode, constructorContext, referenceDeclarations, userDefinedTypes, abiAllocations, compilationId, compiler) {
    let allocations = {
    ...
    for (let abiEntry of abi) {
        ...
        switch (abiEntry.type) {
            case "constructor":
                allocations.constructorAllocation = allocateCalldataAndReturndata(abiEntry, contractNode, referenceDeclarations, userDefinedTypes, abiAllocations, compilationId, compiler, constructorContext);
                break;
            case "function":
                allocations.functionAllocations[AbiDataUtils.abiSelector(abiEntry)] = allocateCalldataAndReturndata(abiEntry, contractNode, referenceDeclarations, userDefinedTypes, abiAllocations, compilationId, compiler, constructorContext);
                break;
            default:
...
const constructorOutputAllocation = {
    ...
    allocationMode: "full",
    kind: "bytecode",
    arguments: [] //included for type niceness but ignored
};
function getCalldataAllocations(contracts, referenceDeclarations, userDefinedTypes, abiAllocations) {
    ...
    for (let contract of contracts) {
        const contractAllocations = getCalldataAllocationsForContract(contract.abi, contract.contractNode, contract.constructorContext, referenceDeclarations[contract.compilationId], userDefinedTypes, abiAllocations, contract.compilationId, contract.compiler);
