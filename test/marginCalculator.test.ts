...
contract('MarginCalculator', () => {
  ...
  describe('Is valid state tests', () => {
    describe('Vault prerequisites checks.', () => {
      it('Should revert when vault contains more than 1 short', async () => {
        const vault = {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Too many short otokens in the vault.')
      })
      it('Should revert when vault contain more than 1 long asset', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Too many long otokens in the vault.')
      })
      it('Should revert when vault contain more than 1 collateral asset', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Too many collateral assets in the vault.')
      })
      ...
      it('Should revert when short assets and amounts have differenct length', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Short asset and amount mismatch')
      })
      it('Should revert when long assets and amounts have differenct length', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Long asset and amount mismatch')
      })
      it('Should revert when collateral assets and amounts have differenct length', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Collateral asset and amount mismatch')
      })
      ...
      it("Should return collateral amount if there's no short.", async () => {
        ...
        const isValid = await calculator.isValidState(vault)
      ...
      it('Should revert if long token has differet underlying as short.', async () => {
        ...
        await expectRevert(calculator.isValidState(vault), 'MarginCalculator: Long and short batch mismatch')
      })
      ...
      //TODO: what should we do if no shortotkoken but yes shortAmount?!
      it('Should return true if no shortOtoken', async () => {
        const vault = createVault(
          undefined,
          eth250Put.address,
          undefined,
      ...
      // it('Should return ?? if no shortOtoken but yes', async () => {
      //   const otokenWrongExpiry = await MockOtoken.new()
      //   await otokenWrongExpiry.init(weth.address, usdc.address, usdc.address, '0', expiry + 1, true)
      //   const vault = createVault(
      //     eth250Put.address,
    ...
    describe('Put vault check before expiry', () => {
      ...
      it('(5) Short: 1 250 put, long 1 200 put => need 50 collatearl', async () => {
        ...
        const isValid = await calculator.isValidState(vaultWithoutCollateral)
        assert.equal(isValid, false)
      })
      ...
      it('(7) Short: 1 250 put, long: 1 200 put, collateral: 50 usdc => excess: 0', async () => {
        ...
        const isValid = await calculator.isValidState(vaultWithCollateral)
        assert.equal(isValid, true)
      })
