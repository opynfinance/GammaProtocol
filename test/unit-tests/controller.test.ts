...
contract('Controller', ([owner, accountOwner1, accountOwner2, accountOperator1, holder1, pauser, random, donor]) => {
  // ERC20 mock
  ...
  describe('Vault', () => {
    it('should get vault', async () => {
      ...
      await controllerProxy.getVaultWithDetails(accountOwner1, vaultId)
    })
  ...
  describe('Long otoken', () => {
    ...
    describe('deposit long otoken', () => {
      ...
      it('should deposit long otoken into vault from account operator', async () => {
        ...
        const vaultBefore = (await controllerProxy.getVaultWithDetails(accountOwner1, vaultCounter))[0]
    ...
    describe('withdraw long otoken', () => {
      ...
      describe('withdraw expired long otoken', () => {
        ...
        before(async () => {
          ...
          await expiredLongOtoken.approve(marginPool.address, longToDeposit, {from: accountOwner1})
          await controllerProxy.operate(actionArgs, {from: accountOwner1})
          const vaultAfter = (await controllerProxy.getVaultWithDetails(accountOwner1, vaultId))[0]
          assert.equal(vaultAfter.longOtokens.length, 1, 'Vault long otoken array length mismatch')
          assert.equal(
        ...
        it('should revert withdrawing an expired long otoken', async () => {
          ...
          const vaultId = new BigNumber('1')
          const vault = (await controllerProxy.getVaultWithDetails(accountOwner1, vaultId))[0]
  ...
  describe('Settle vault', () => {
    ...
    it('should settle vault with only long otokens in it', async () => {
      ...
      await oracle.setExpiryPriceFinalizedAllPeiodOver(weth.address, expiry, createScaledNumber(ethPriceAtExpiry), true)
  ...
  describe('Pause mechanism', () => {
    ...
    it('should revert set pauser address from non-owner', async () => {
      await expectRevert(controllerProxy.setPauser(pauser, {from: random}), 'Ownable: caller is not the owner')
    })
    ...
    it('should revert set pauser address to the same previous address', async () => {
      await expectRevert(controllerProxy.setPauser(await controllerProxy.pauser(), {from: owner}), 'CO9')
    })
    ...
    it('should revert partially un-pausing an already running paused system', async () => {
      await expectRevert(controllerProxy.setSystemPartiallyPaused(false, {from: pauser}), 'CO9')
    })
    ...
    it('should pause system', async () => {
      ...
      await controllerProxy.setSystemPartiallyPaused(true, {from: pauser})
    ...
    it('should revert partially pausing an already patially paused system', async () => {
      await expectRevert(controllerProxy.setSystemPartiallyPaused(true, {from: pauser}), 'CO9')
    })
  ...
  describe('Full Pause', () => {
    ...
    before(async () => {
      ...
      await controllerProxy.setSystemPartiallyPaused(false, {from: pauser})
    ...
    it('should revert fully un-pausing an already running system', async () => {
      await expectRevert(controllerProxy.setSystemFullyPaused(false, {from: pauser}), 'CO9')
    })
    ...
    it('should trigger full pause', async () => {
      ...
      await controllerProxy.setSystemFullyPaused(true, {from: pauser})
    ...
    it('should revert fully pausing an already fully paused system', async () => {
      await expectRevert(controllerProxy.setSystemFullyPaused(true, {from: pauser}), 'CO9')
    })
  ...
  describe('Donate to pool', () => {
    it('it should donate to margin pool', async () => {
      ...
      assert.equal(storedBalanceAfter.minus(storedBalanceBefore).toString(), amountToDonate, 'Donated amount mismatch')
    })
  })
