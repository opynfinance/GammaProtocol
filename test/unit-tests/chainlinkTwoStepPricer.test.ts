...
contract('ChainlinkTwoStepPricer', ([owner, bot, random]) => {
  ...
  before('Deployment', async () => {
    ...
    // deploy pricer
    pricer = await ChainlinkTwoStepPricer.new(bot, weth.address, wethAggregator.address, oracle.address)
  })
  ...
  describe('constructor', () => {
    ...
    it('should revert if initializing aggregator with 0 address', async () => {
      await expectRevert(
        ChainlinkTwoStepPricer.new(bot, weth.address, ZERO_ADDR, wethAggregator.address),
        'ChainLinkTwoStepPricer: Cannot set 0 address as aggregator',
      )
    })
    it('should revert if initializing oracle with 0 address', async () => {
      await expectRevert(
        ChainlinkTwoStepPricer.new(bot, weth.address, oracle.address, ZERO_ADDR),
        'ChainLinkTwoStepPricer: Cannot set 0 address as oracle',
      )
    })
    it('should revert if initializing bot with 0 address', async () => {
      await expectRevert(
        ChainlinkTwoStepPricer.new(ZERO_ADDR, weth.address, oracle.address, wethAggregator.address),
        'ChainLinkTwoStepPricer: Cannot set 0 address as bot',
      )
    })
  })
  ...
  describe('getPrice', () => {
    ...
    it('should revert if price is lower than 0', async () => {
      ...
      await expectRevert(pricer.getPrice(), 'ChainLinkTwoStepPricer: price is lower than 0')
    })
  })
  ...
  describe('setExpiryPrice', () => {
    ...
    it('should revert if sender is not bot address and roundId is too old', async () => {
      ...
      await expectRevert(
        ...
        'ChainLinkTwoStepPricer: roundId not first after expiry',
      )
    })
    ...
    it('should revert if sender is not bot address and roundId is too late', async () => {
      ...
      await expectRevert(
        pricer.setExpiryPriceInOracle(expiryTimestamp, roundId, { from: random }),
        'ChainLinkTwoStepPricer: previousRoundId not last before expiry',
      )
    })
