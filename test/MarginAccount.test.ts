...
contract('MarginAccount', ([deployer, controller]) => {
  ...
  describe('Otoken Initialization', () => {
    ...
    //     await expectRevert(marginAccountTester.testAddShort(otoken.address, 10, 1), "MarginAccount: invalid short otoken position")
    ...
    //
    //   it('should revert if trying to add wrong otoken to an index', async () => {
    //     await expectRevert(marginAccountTester.testAddLong(otoken.address, 10, 1), "MarginAccount: invalid long otoken position")
    //     const vault = await marginAccountTester.getVault()
    //   })
    //
    ...
    //     let beforeVault = await marginAccountTester.getVault()
    //     let changeAmt = 20
    //     let index = 0
    //     await marginAccountTester.testAddCollateral(weth.address, changeAmt, index)
    //     let afterVault = await marginAccountTester.getVault()
    ...
    //     await expectRevert(marginAccountTester.testAddCollateral(usdc.address, 10, 0), "MarginAccount: invalid collateral token position")
    //   })
    //
    ...
    //      await expectRevert(marginAccountTester.testRemoveShort(otoken.address, 10, 0), "MarginAccount: short otoken address mismatch")
    //   })
    //
    ...
    //     assert.equal((new BigNumber(beforeVault.shortAmounts[index]).minus(new BigNumber(afterVault.shortAmounts[index]))).toString(), new BigNumber(toRemove).toString(),"amount removed mismatch")
    //   })
    //
    //   it('should not be able to remove more than the num of remaining of those short otokens', async () => {
    //     await expectRevert(marginAccountTester.testRemoveShort(otoken.address, 100, 0), "SafeMath: subtraction overflow")
    //   })
    //
    ...
    //     await expectRevert(marginAccountTester.testRemoveShort(otoken2.address, 1, 0), "MarginAccount: short otoken address mismatch")
    //   })
    //
    // })
    //
    ...
    //     await expectRevert(marginAccountTester.testRemoveLong(otoken.address, 10, 0), "MarginAccount: long otoken address mismatch")
    //   })
    //
    ...
    //     account = await marginAccountTester.getAccount({from: deployer})
    //     assert.equal(account.owner, testAccount.owner, 'MarginAccount.Account owner addr mismatch')
    //     assert.equal(account.vaultIds.toString(), testAccount.vaultIds.toString(), 'MarginAccount.Account owner vaultIds mismatch')
    //   })
    //
    //   it('should not remove short collateral if there is none', async () => {
    //     await expectRevert(marginAccountTester.testRemoveCollateral(weth.address, 10, 0), "MarginAccount: collateral token address mismatch")
    //   })
    //
    ...
    //     await expectRevert(marginAccountTester.testRemoveCollateral(weth.address, 10, 1),"MarginAccount: collateral token address mismatch")
    //   })
    //
    //   it('should not be able to remove more than the amount of collateral', async () => {
    //     await expectRevert(marginAccountTester.testRemoveCollateral(otoken.address, 100, 1),  "MarginAccount: collateral token address mismatch")
    //   })
    //
    //   //TODO: Is this a condition we need?
    //   // it('should not be able to remove zero of those short otokens', async () => {
