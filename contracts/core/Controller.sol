 ...
 * CO9: invalid input
 * CO1: fullPauser cannot be set to address zero
 * COB: partialPauser cannot be set to address zero
 * COC: can not run actions for different owners
 * COD: can not run actions on different vaults
 * COE: invalid final vault state
 * COF: can not run actions on inexistent vault
 ...
 * CO8: can not redeem un-expired otoken
 ...
 * CO4: can not leave less than collateral dust
...
contract Controller is Initializable, OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe {
    ...
    modifier onlyPartialPauser {
        require(msg.sender == partialPauser, "A");
    ...
    modifier onlyWhitelistedCallee(address _callee) {
        if (callRestricted) {
            require(_isCalleeWhitelisted(_callee), "2");
        }
    ...
    function _isNotPartiallyPaused() internal view {
        require(!systemPartiallyPaused, "3");
    }
    ...
    function _isAuthorized(address _sender, address _accountOwner) internal view {
        require((_sender == _accountOwner) || (operators[_accountOwner][_sender]), "5");
    }
    ...
    function initialize(address _addressBook, address _owner) external initializer {
        require(_addressBook != address(0), "6");
        require(_owner != address(0), "7");
    ...
    function setPartialPauser(address _partialPauser) external onlyOwner {
        require(_partialPauser != address(0), "10");
    ...
    function setCallRestriction(bool _isRestricted) external onlyOwner {
        require(callRestricted != _isRestricted, "8");
    ...
    function setOperator(address _operator, bool _isOperator) external {
        require(operators[msg.sender][_operator] != _isOperator, "8");
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _actions.length; i++) {
            ...
            if (
                ...
                if (vaultUpdated) {
                    require(vaultOwner == action.owner, "B");
                    require(vaultId == action.vaultId, "C");
                }
                vaultUpdated = true;
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        require(_args.vaultId == vaultId, "E");
    ...
    function _depositLong(Actions.DepositArgs memory _args)
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        require(_checkVaultId(_args.owner, _args.vaultId), "4");
        // only allow vault owner or vault operator to deposit long otoken
        require((_args.from == msg.sender) || (_args.from == _args.owner), "F");

        require(whitelist.isWhitelistedOtoken(_args.asset), "16");

        OtokenInterface otoken = OtokenInterface(_args.asset);

        require(now < otoken.expiryTimestamp(), "17");
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        require(now < otoken.expiryTimestamp(), "18");
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        require((_args.from == msg.sender) || (_args.from == _args.owner), "19");

        require(whitelist.isWhitelistedCollateral(_args.asset), "20");
        ...
        if (typeVault == 1) {
            ...
            require(nakedPoolBalance[_args.asset] <= nakedCap[_args.asset], "36");
        }
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        if (_isNotEmpty(vault.shortOtokens)) {
            ...
            require(now < otoken.expiryTimestamp(), "21");
        }
    ...
    {
        require(_checkVaultId(_args.owner, _args.vaultId), "4");
        require(whitelist.isWhitelistedOtoken(_args.otoken), "22");

        OtokenInterface otoken = OtokenInterface(_args.otoken);

        require(now < otoken.expiryTimestamp(), "23");
        ...
        onlyAuthorized(msg.sender, _args.owner)
    {
        ...
        require((_args.from == msg.sender) || (_args.from == _args.owner), "24");

        OtokenInterface otoken = OtokenInterface(_args.otoken);

        // do not allow burning expired otoken
        require(now < otoken.expiryTimestamp(), "25");
    ...
    function _redeem(Actions.RedeemArgs memory _args) internal {
        ...
        require(whitelist.isWhitelistedOtoken(_args.otoken), "26");
        ...
        require(now >= expiry, "27");

        require(isSettlementAllowed(underlying, strike, collateral, expiry), "8");
    ...
    function _liquidate(Actions.LiquidateArgs memory _args) internal notPartiallyPaused {
        ...
        if (vault.shortAmounts[0].sub(_args.amount) > 0) {
            require(vault.collateralAmounts[0].sub(collateralToSell) >= collateralDust, "33");
        }
